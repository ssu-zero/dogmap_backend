...
from app.domains.auth.service import KakaoRedirectUriError, login_with_kakao
...
async def kakao_login(body: KakaoLoginRequest, db: Session = Depends(get_db)):
    try:
        redirect_uri = str(body.redirect_uri) if body.redirect_uri is not None else None
        return await login_with_kakao(db, body.code, redirect_uri)
    except (KakaoOAuthError, KakaoRedirectUriError) as exc:
